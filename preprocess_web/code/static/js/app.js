...
$(document).ready(function () {
	...
	function disply_information(d, i) {
		...
		for (var i in all_obj['variables'][varname]) {
			char_list.push({
				name: i,
				value: all_obj['variables'][varname][i]
			})
		}
		...
		if (all_obj['variables'][varname]['plottype'] == 'bar') {
			bar_cross(all_obj['variables'][varname], varname)
	...
	function density_cross(density_env) {
		...
		for (var i = 0; i < density_env.plotx.length; i++) {
			data2.push({
				x: density_env.plotx[i],
				y: density_env.ploty[i]
			});
		}
	...
	function bar_cross(bar_env, varname) {
		document.getElementById("plots").innerHTML = "";
		...
		var margin_cross = {
			...
			width_cross = 400 - margin_cross.left - margin_cross.right,
			height_cross = 280 - margin_cross.top - margin_cross.bottom;
		...
		for (var i = 0; i < keys.length; i++) {
			...
			// console.log("THis ",xVals[i]+":"+yVals[i])
		...
		var xAxis = d3.axisBottom(x_1)
			.ticks(yVals.length);

		var yAxis = d3.axisLeft(y_1);

		var plotsvg1 = d3.select("#plots")
			.append("svg")
			.attr("id", "plotsvg1_id")
			.style("width", width_cross + margin_cross.left + margin_cross.right) //setting height to the height of #main.left
			.style("height", height_cross + margin_cross.top + margin_cross.bottom)
		...
		plotsvg1.append("text")
			.attr("x", (width_cross / 2))
			.attr("y", margin_cross.top + (2 * padding_cross))
			.attr("text-anchor", "middle")
			.text(varname)
			.style("text-indent", "20px")
			.style("font-size", "12px")
